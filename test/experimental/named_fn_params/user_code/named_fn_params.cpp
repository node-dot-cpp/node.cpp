...
int main()
{
...
#endif // 0
	{
		...
		nodecpp::vector<Point> vectorOfPoints;
		nodecpp::vector<Point3D> vectorOfPoints3D = { {0, 1, 2}, {3, 4, 5} };
		auto point3Dit = vectorOfPoints3D.begin();
		nodecpp::vector<int> vectorOfNumbersBack;
		nodecpp::vector<Point> vectorOfPointsBack;
		nodecpp::vector<Point3D> vectorOfPoints3DBack;
		nodecpp::Buffer b;
		m::Composer composer( m::Proto::GMQ, b );
//		m::message_one_compose( composer, m::sixthParam = m::CollectionWrapperForComposing( vectorOfPoints3D/*, [&vectorOfPoints3D]() { return vectorOfPoints3D.size(); }*/, [](m::Composer c, Point3D& p){m::point3D_compose( c, m::x = p.x, m::y = p.y, m::z = p.z );} ), m::firstParam = 1, m::secondParam = nodecpp::string("def"), m::thirdParam = 3, m::forthParam = m::SimpleTypeCollectionWrapper( vectorOfNumbers ) );
		m::message_one_compose( composer, m::sixthParam = m::CollectionWrapperForComposing( vectorOfPoints3D/*, [&vectorOfPoints3D]() { return vectorOfPoints3D.size(); }*/, [&vectorOfPoints3D, &point3Dit](m::Composer c){ if ( point3Dit != vectorOfPoints3D.end() ) { m::point3D_compose( c, m::x = point3Dit->x, m::y = point3Dit->y, m::z = point3Dit->z ); ++point3Dit; return point3Dit != vectorOfPoints3D.end();} return false;} )/*, m::firstParam = 1, m::secondParam = nodecpp::string("def"), m::thirdParam = 3, m::forthParam = m::SimpleTypeCollectionWrapper( vectorOfNumbers )*/ );
