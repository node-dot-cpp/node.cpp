...
{
	{
		nodecpp::vector<Point> vec = { {0, 1}, {2, 3}, {4, 5} };
		nodecpp::vector<Point> vecout;
		nodecpp::Buffer b;
//		man::message_one_compose( b, man::firstParam = 1, man::secondParam = nodecpp::string("def"), man::thirdParam = 3, man::forthParam = [vec]( Buffer&b, size_t idx){ if ( idx >= vec.size() ) return 0; man::point_compose( b, man::x = vec[idx].x, man::y = vec[idx].y ); return 1; } );
		man::message_one_compose( b, man::firstParam = 1, man::secondParam = nodecpp::string("def"), man::thirdParam = 3, man::forthParam = [vec]( Buffer&b, size_t idx){ if ( idx >= vec.size() ) return 0; int xx = vec[idx].x; int yy = vec[idx].y; man::point_compose( b, man::x = xx, man::y = yy ); return 1; } );

		Parser parser( b.begin(), b.size() );
		int firstParam = -1;
		nodecpp::string secondParam = "";
		int thirdParam = -1;
		man::message_one_parse( parser, man::firstParam = &firstParam, man::secondParam = &secondParam, man::thirdParam = &thirdParam, man::forthParam = [&vec](size_t idx){ return 0; } );
